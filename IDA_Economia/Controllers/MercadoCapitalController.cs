...
namespace IDA_Economia.Controllers
{
    public class MercadoCapitalController : Controller
    {
        ...
        [HttpPost]
        public JsonResult ObtenerEstadistico()
        {
            ...
            fechaInicio = Convert.ToDateTime("2019/06/01");
            fechafin = Convert.ToDateTime("2019/06/30");

            totalDias = (fechafin - fechaInicio).TotalDays;
            ...
            ListaEmpresa.ForEach(x =>
            {
                ...
                List<YahooHistoricalPriceData> yahooPriceHistory = yahooFinance.GetDailyHistoricalPriceData(yahooStockCode, fechaInicio, fechafin);
            ...
            //Chart3.ChartAreas[0].AxisX.Title = "Riesgo del portafolio";
            //Chart3.ChartAreas[0].AxisY.Title = "Rendimiento del portafolio";

            ////Chart3.ChartAreas[0].AxisY.Minimum = 0;

            //cont = 0;

            //foreach (CurvaVarianza curva in ListaCurvaVarianza)
            //{
            //    DataPoint data = new DataPoint();

            //    data.SetValueXY(curva.Sigma, new object[] { curva.RendimientoAsumido });

            //    Chart3.Series[1].Points.Add(data);

            //    Chart3.Series[1].Points[cont].MarkerColor = System.Drawing.Color.Yellow;

            //    Chart3.Series[1].Points[cont].IsValueShownAsLabel = false;

            //    Chart3.Series[1].Points[cont].Label = "[" + (cont + 1) + "]";

            //    Chart3.Series[1].Points[cont].IsVisibleInLegend = false;

            //    //Chart3.Series[1].Points[cont].AxisLabel = "[" + cont + 1 + "]";

            //    Chart3.Series[0].Points.Add(data);

            //    Chart3.Series[0].Points[cont].MarkerColor = System.Drawing.Color.Blue;

            //    //Chart3.Series[0].Points[cont].Label = "";
            ...
            //htmlScript.AppendLine("$('#collapseUno').collapse('show');");
            //htmlScript.AppendLine("$('#collapseDos').collapse('hide');");
            //htmlScript.AppendLine("$('#collapseTres').collapse('hide');");

            //if (ListaCurvaVarianza.ToList().Count > 0)
            //{
            //    MensajeCorrecto("Mercado de Capitales", "Se realizo correctamente el calculo.", htmlScript.ToString());
            //}
            //else
            //{
